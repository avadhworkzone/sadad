...
class OtherDocumentUploadNewFlowScreen extends StatefulWidget {
  String? businessStatus;
  List<OwnerIdUploadModel> allOwnerIdCardListForwarded;

  OtherDocumentUploadNewFlowScreen({Key? key, this.businessStatus, required this.allOwnerIdCardListForwarded}) : super(key: key);

  @override
  State<OtherDocumentUploadNewFlowScreen> createState() => _OtherDocumentUploadNewFlowScreenState();
}

class _OtherDocumentUploadNewFlowScreenState extends State<OtherDocumentUploadNewFlowScreen> {
          ...
          children: [
            Expanded(
              child: Column(
                ...
                children: [
                  customMediumBoldText(title: 'Other'.tr),
                  height5(),
                  customVerySmallNorText(title: "Upload other type of document here".tr, color: ColorsUtils.grey),
                ],
              ),
            ),
          ],
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          customSmallBoldText(title: '${index + 1}. '+'Other'.tr),
                                          width10(),
                                                    ...
                                                    if (allOwnerIdCardListMain[index].image!.length + allOwnerIdCardListMain[index].onlineImage!.length > 0) {
                                                      showDialog<String>(
                                                          ...
                                                          builder: (BuildContext context) {
                                                            return AlertDialog(
                                                              title: Text('Are you sure?'.tr),
                                                              content: Text('Do you want to remove this Other document?'.tr),
                                                              actions: <Widget>[
                                                                TextButton(
                                                                  child: Text('No'.tr),
                                                                  onPressed: () => Navigator.pop(context),
                                        ...
                                        child: ListView.builder(
                                            ...
                                            itemBuilder: (context, subIndex) {
                                              return ((allOwnerIdCardListMain[index].image?.length ?? 0) + (allOwnerIdCardListMain[index].onlineImage?.length ?? 0) == 0 ||
                                                  ...
                                                  ? ((allOwnerIdCardListMain[index].image?.length ?? 0) + (showUploadButton(index))) > 4
                                                      ? SizedBox()
                                                                          ...
                                                                          onTap: () {
                                                                            ...
                                                                            if (!docUploadLimitReached(index)) {
                                                                              ...
                                                                              allOwnerIdCardListMain[index].errorMessage =
                                                                                  "Maximum number of page limits has been reached. You're allowed to upload only 5 pages for Other document category.".tr;
                    ...
                    child: Container(
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            customSmallNorText(
                              title: 'Upload'.tr+' ${numberToWord(allOwnerIdCardListMain.length + 1)} '+'Other document'.tr,
                              color: ColorsUtils.grey,
                            ),
                            Container(
                              decoration: BoxDecoration(
        ...
        result.files.forEach((singleFile) async {
          ...
          if (result.files.last == singleFile) {
            Future.delayed(Duration(milliseconds: 300), () {
              if (allOwnerIdCardListMain[index].image!.length + showUploadButton(index) > 5) {
                allOwnerIdCardListMain[index].isDocVerified = false;
                allOwnerIdCardListMain[index].errorMessage = "Maximum number of page limits has been reached. You're allowed to upload only 5 pages for Other document category.".tr;
