...
class EstablishmentCardUploadNewFlowScreen extends StatefulWidget {
  String? businessStatus;
  List<OwnerIdUploadModel> allOwnerIdCardListForwarded;

  EstablishmentCardUploadNewFlowScreen({Key? key, this.businessStatus, required this.allOwnerIdCardListForwarded}) : super(key: key);

  @override
  State<EstablishmentCardUploadNewFlowScreen> createState() => _EstablishmentCardUploadNewFlowScreenState();
}

class _EstablishmentCardUploadNewFlowScreenState extends State<EstablishmentCardUploadNewFlowScreen> {
          ...
          children: [
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  customMediumBoldText(title: 'Establishment card'.tr),
                  height5(),
                  customVerySmallNorText(title: "Upload your valid Company Establishment card ".tr, color: ColorsUtils.grey),
                ],
              ),
            ),
            Column(
                ...
                Row(
                  children: [
                    Container(
                      ...
                      child: Image.asset(Images.establishment_Sample),
                    ),
                  ],
                ),
              ],
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          customMediumBoldText(title: '${index + 1}. '+'Establishment card'.tr),
                                          width20(),
                                                    ...
                                                    if (allOwnerIdCardListMain[index].image!.length + allOwnerIdCardListMain[index].onlineImage!.length > 0) {
                                                      showDialog<String>(
                                                          ...
                                                          builder: (BuildContext context) {
                                                            return AlertDialog(
                                                              title: Text('Are you sure?'.tr),
                                                              content: Text('Do you want to remove this Establishment card?'.tr),
                                                              actions: <Widget>[
                                                                TextButton(
                                                                  child: Text('No'.tr),
                                                                  onPressed: () => Navigator.pop(context),
                                                                          ...
                                                                          onTap: () {
                                                                            ...
                                                                            if (!docUploadLimitReached(index)) {
                                                                              ...
                                                                              allOwnerIdCardListMain[index].errorMessage =
                                                                                  "Maximum number of page limits has been reached. You're allowed to upload only 2 pages for Establishment card document category.".tr;
                    ...
                    child: Container(
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            customSmallNorText(
                              title: 'Upload'.tr+' ${numberToWord(allOwnerIdCardListMain.length + 1)} '+'Establishment card'.tr,
                              color: ColorsUtils.grey,
                            ),
                            Container(
                              decoration: BoxDecoration(
          ...
          if (result.files.last == singleFile) {
            Future.delayed(Duration(milliseconds: 300), () {
              if (allOwnerIdCardListMain[index].image!.length + showUploadButton(index) > 2) {
                allOwnerIdCardListMain[index].isDocVerified = false;
                allOwnerIdCardListMain[index].errorMessage = "Maximum number of page limits has been reached. You're allowed to upload only 2 pages for Establishment card document category.".tr;
  ...
  verifyBusinessDoc(String imageFile, int index, int totalPages) async {
    ...
    if (totalPages == newDocCounter) {
      ...
      //   allOwnerIdCardListMain[index].isDocVerified = false;
      //   allOwnerIdCardListMain[index].errorMessage = "You have uploaded".tr+" ${otherDocNameInList.toString()} "+"document with valid type document. Please remove it.";
      // } else if (otherDocNameInList != null && otherDocNameInList.length > 0) {
      //   allOwnerIdCardListMain[index].isDocVerified = false;
      //   allOwnerIdCardListMain[index].errorMessage =
      //       "You have uploaded".tr+" ${otherDocNameInList.toString()} "+"document in Establishment card document category.".tr+"Kindly remove this document from Establishment card category.".tr;
      // } else if (!verified) {
      //   allOwnerIdCardListMain[index].isDocVerified = false;
      //   allOwnerIdCardListMain[index].errorMessage = "Sorry! You didn't upload a valid Establishment card document.Kindly reupload the document.".tr;
      ...
      //   allOwnerIdCardListMain[index].isDocVerified = false;
      //   allOwnerIdCardListMain[index].errorMessage = "You have uploaded two different Establishment card. Please upload single Establishment card";
      // } else if (multipleDocWithSameType > 1) {
      //   allOwnerIdCardListMain[index].isDocVerified = false;
      //   allOwnerIdCardListMain[index].errorMessage = "You have uploaded multiple Establishment card document. Kindly upload only one document here.".tr;
      // }
  ...
  bool allowedToAddAnotherId() {
    bool show = true;
    if (allOwnerIdCardListMain.length < 1) {
      show = true;
    } else {
      show = false;
    }
    if (allOwnerIdCardListMain.length > 0) {
      if (allOwnerIdCardListMain.last.isDocVerified == true && allOwnerIdCardListMain.length < 1) {
        show = true;
      } else {
        show = false;
      }
