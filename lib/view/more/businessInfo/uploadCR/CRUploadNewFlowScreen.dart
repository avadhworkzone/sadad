...
class CRUploadNewFlowScreen extends StatefulWidget {
  String? businessStatus;
  List<OwnerIdUploadModel> allOwnerIdCardListForwarded;

  CRUploadNewFlowScreen({Key? key, this.businessStatus, required this.allOwnerIdCardListForwarded}) : super(key: key);

  @override
  State<CRUploadNewFlowScreen> createState() => _CRUploadNewFlowScreenState();
}

class _CRUploadNewFlowScreenState extends State<CRUploadNewFlowScreen> {
          ...
          children: [
            Expanded(
              child: Column(
                ...
                children: [
                  customMediumBoldText(title: 'Commercial registration'.tr),
                  height5(),
                  customVerySmallNorText(title: "Upload your valid Commercial Registration copy as issued by the Ministry of Economy & Commerce".tr, color: ColorsUtils.grey),
                ],
              ),
            ),
            Column(
                ...
                Row(
                  children: [
                    Container(
                      ...
                      child: Image.asset(Images.cr_Sample),
                    ),
                  ],
                ),
              ],
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          customSmallBoldText(title: '${index + 1}. ' + 'Commercial registration'.tr),
                                          width10(),
                                                    ...
                                                    if (allOwnerIdCardListMain[index].image!.length + allOwnerIdCardListMain[index].onlineImage!.length > 0) {
                                                      showDialog<String>(
                                                          ...
                                                          builder: (BuildContext context) {
                                                            return AlertDialog(
                                                              title: Text('Are you sure?'.tr),
                                                              content: Text('Do you want to remove this Commercial registration?'.tr),
                                                              actions: <Widget>[
                                                                TextButton(
                                                                  child: Text('No'.tr),
                                                                  onPressed: () => Navigator.pop(context),
                                        ...
                                        child: ListView.builder(
                                            ...
                                            itemBuilder: (context, subIndex) {
                                              return ((allOwnerIdCardListMain[index].image?.length ?? 0) + (allOwnerIdCardListMain[index].onlineImage?.length ?? 0) == 0 ||
                                                  ...
                                                  ? ((allOwnerIdCardListMain[index].image?.length ?? 0) + (showUploadButton(index))) > 9
                                                      ? SizedBox()
                                      ...
                                      height12(),
                                      allOwnerIdCardListMain[index].image!.length + allOwnerIdCardListMain[index].onlineImage!.length > 1
                                          ? Row(
                                              children: [
                                                width20(),
                                                customVerySmallNorText(title: 'Selected pages :'.tr + ' ${countTotalDoc(index) + allOwnerIdCardListMain[index].image!.length}'),
                                              ],
                                            )
                                          : SizedBox(),
                                      height12(),
                    ...
                    child: Container(
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            customSmallNorText(
                              title: 'Upload'.tr + ' ${numberToWord(allOwnerIdCardListMain.length + 1)} ' + 'Commercial registration'.tr,
                              color: ColorsUtils.grey,
                            ),
                            Container(
                              decoration: BoxDecoration(
        ...
        result.files.forEach((singleFile) async {
          ...
          if (result.files.last == singleFile) {
            Future.delayed(Duration(milliseconds: 300), () {
              if (allOwnerIdCardListMain[index].image!.length + showUploadButton(index) > 10) {
                allOwnerIdCardListMain[index].isDocVerified = false;
                allOwnerIdCardListMain[index].errorMessage = "Maximum number of page limits has been reached. You're allowed to upload only 10 pages for Commercial registration document category.".tr;
  ...
  verifyBusinessDoc(String imageFile, int index, int totalPages) async {
    ...
    if (totalPages == newDocCounter) {
      ...
      //   allOwnerIdCardListMain[index].isDocVerified = false;
      //   allOwnerIdCardListMain[index].errorMessage =
      //       "You have uploaded".tr+" ${otherDocNameInList.toString()} document in Commercial registration document category.".tr+"Kindly remove this document from Commercial registration category.".tr;
      // } else if (!verified) {
      //   allOwnerIdCardListMain[index].isDocVerified = false;
      //   allOwnerIdCardListMain[index].errorMessage = "Sorry! You didn't upload a valid Commercial registration document.Kindly reupload the document.".tr;
      ...
      //   allOwnerIdCardListMain[index].isDocVerified = false;
      //   allOwnerIdCardListMain[index].errorMessage = "You have uploaded two different Commercial registration. Please upload single Commercial registration";
      // } else if (multipleDocWithSameType > 1) {
      //   allOwnerIdCardListMain[index].isDocVerified = false;
      //   allOwnerIdCardListMain[index].errorMessage = "You have uploaded multiple Commercial registration card document. Kindly upload only one document here.".tr;
      // }
  ...
  VerifyOnlineImages(int index) {
    if (countTotalDoc(index) > 0) {
      ...
      // } else if (multipleDocWithSameType > 1) {
      //   allOwnerIdCardListMain[index].isDocVerified = false;
      //   allOwnerIdCardListMain[index].errorMessage = "You have uploaded multiple Commercial registration document. Kindly upload only one document here.".tr;
      // }
      // // else if (uploadedOwnerId()) {
      // //   addButtonDisable = true;
      // //   currentErrorMsg = "You have already uploaded this document so can not upload same document twice.";
  ...
  bool allowedToAddAnotherId() {
    bool show = true;
    if (allOwnerIdCardListMain.length < 2) {
      show = true;
    } else {
      show = false;
    }
    if (allOwnerIdCardListMain.length > 0) {
      if (allOwnerIdCardListMain.last.isDocVerified == true && allOwnerIdCardListMain.length < 2) {
        show = true;
      } else {
        show = false;
      }
