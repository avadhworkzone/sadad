...
class _ReportDetailScreenState extends State<ReportDetailScreen> {
  ReportDetailViewModel reportDetailViewModel = Get.find();
  TransactionReportResponseModel? reportDetailResponseModel;
  ...
  Widget build(BuildContext context) {
    if (connectivityViewModel.isOnline != null) {
      if (connectivityViewModel.isOnline!) {
        return type == 1
            ? Scaffold(body: GetBuilder<ReportDetailViewModel>(
                builder: (controller) {
                  ...
                  reportDetailResponseModel =
                      reportDetailViewModel.transactionReportApiResponse.data;
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  // smallSemiBoldText(
                                  //     text:
                                  //         '${DateFormat('dd MMM yy').format(DateTime.parse(widget.startDate!))}'),
                                ],
                              ),
                            ),
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  // smallSemiBoldText(
                                  //     text:
                                  //         '${DateFormat('dd MMM yy').format(DateTime.parse(widget.endDate!))}'),
                                ],
                              ),
                            ),
                            width50(),
                          ...
                          if (isRadioSelected == 0) {
                            ...
                            if (sendEmail == true) {
                              String token = await encryptedSharedPreferences
                                  .getString('token');
                              final url = Uri.parse(
                                '${Utility.baseUrl}reporthistories/transactionDetailsReport?filter[where][created][between][0]=${widget.startDate}&filter[where][created][between][1]=${intl.DateFormat('yyyy-MM-dd').format(DateTime.parse(widget.endDate!))} 23:59:59&filter[order]=created%20DESC&${isRadioSelected == 1 ? 'isPdf' : isRadioSelected == 2 ? 'isCsv' : 'isExcel'}=true&isEmail=true',
                              );
                              ...
                              await downloadFile(
                                ...
                                url:
                                    '${Utility.baseUrl}reporthistories/transactionDetailsReport?filter[where][created][between][0]=${widget.startDate}&filter[where][created][between][1]=${intl.DateFormat('yyyy-MM-dd').format(DateTime.parse(widget.endDate!))} 23:59:59&filter[order]=created%20DESC&${isRadioSelected == 1 ? 'isPdf' : isRadioSelected == 2 ? 'isCsv' : 'isExcel'}=true',
                                context: context,
                              );
                            }
                          }
  ...
  void initData() async {
    ...
    widget.selectedType == 'transactionDetailsReport'
        ? type = 1
        : widget.selectedType == 'settlementReports'
            ? type = 2
            : widget.selectedType == 'invoiceReports'
                ? type = 3
                : widget.selectedType == 'subscriptionDetailsReports'
                    ? type = 4
                    : widget.selectedType == 'orderReports'
                        ? type = 5
                        : type = 0;
    setState(() {});
